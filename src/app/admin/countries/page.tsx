...
import {
  Plus, Eye, Edit, Trash2, Search, Tag
} from 'lucide-react';
import prisma from '@/lib/prisma';
import { formatDate } from '@/lib/utils';

export const metadata: Metadata = {
  title: 'Categories Management — TivoAds Admin',
  robots: { index: false },
};
...
export default async function CategoriesPage({ searchParams }: { searchParams: Promise<any> | any }) {
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Manage ad Countries</p>
        </div>
        <Link href="/admin/countries/new" className="btn btn-primary btn-md">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
          ...
          <div className="text-xs text-white/50 mt-1">Total Countries</div>
        </div>
        <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
          <div className="text-2xl font-bold text-white">{categories.filter(c => c._count.ads > 0).length}</div>
          <div className="text-xs text-white/50 mt-1">With Ads</div>
        </div>
        <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
          <div className="text-2xl font-bold text-white">{categories.reduce((sum, c) => sum + c._count.ads, 0)}</div>
          <div className="text-xs text-white/50 mt-1">Total Ads</div>
        </div>
        <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
          <div className="text-2xl font-bold text-white">{categories.filter(c => c._count.ads === 0).length}</div>
          <div className="text-xs text-white/50 mt-1">Empty</div>
        </div>
      </div>
            ...
            <tbody>
              {categories.map((category) => (
                <tr key={category.id} className="border-b border-white/10 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-white/60 line-clamp-1">{category.description || 'N/A'}</td>
                  <td className="px-6 py-4 text-center">
                    <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-brand-500/20 text-brand-400">
                      {category._count.ads}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-white/60">{formatDate(category.createdAt)}</td>
